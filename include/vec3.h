...
float vec3_get(Vec3 *v, int i) { return v->e[i]; }

void vec3_set(Vec3 *v, int i, float val) { v->e[i] = val; }
...
float vec3_length(Vec3 v) { return sqrt(vec3_length_squared(v)); }
...
Vec3 vec3_random_interval(float min, float max) {
  return make_vec3(random_float_interval(min, max),
                   random_float_interval(min, max),
                   random_float_interval(min, max));
}
